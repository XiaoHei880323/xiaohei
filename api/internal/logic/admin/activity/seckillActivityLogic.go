...
// ActivityType: 2=秒杀活动
const seckillActivityType = 2

type SeckillActivityLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewSeckillActivityLogic(ctx context.Context, svcCtx *svc.ServiceContext) *SeckillActivityLogic {
	return &SeckillActivityLogic{
...
// 获取秒杀活动列表
func (l SeckillActivityLogic) ActivityListLogic(req activityReq.ActivityListReq) (resp *resp.CommonReply, err error) {
	returnData := reponse.ReturnStruct()
	sql := "activity_type = ?"
	whereSlice := []interface{}{seckillActivityType}
	if req.ActivityName != "" {
...
// 添加秒杀活动
func (l SeckillActivityLogic) ActivityAddLogic(req activityReq.ActivityAddInfoReq, userId int) (resp *resp.CommonReply, err error) {
	returnData := reponse.ReturnStruct()
	insertMap := make(map[string]interface{})
	insertMap["activity_type"] = seckillActivityType
...
// 修改秒杀活动
func (l SeckillActivityLogic) ActivityUpdateLogic(req activityReq.ActivityUpdateInfoReq, userId int) (resp *resp.CommonReply, err error) {
	returnData := reponse.ReturnStruct()
	adminRoleServie := serv.NewAdminUserRoleService(l.ctx, l.svcCtx)
	userRole := adminRoleServie.AmdinRole(userId)
	if userRole != 1 {
		returnData.Code = 100001
		returnData.Message = "当前用户角色不可以进行修改秒杀活动"
		return returnData, nil
	}
	whereActivity := []interface{}{"id = ? and is_delete = ? and activity_type = ?", req.ActivityId, 1, seckillActivityType}
	activityHas, _, activityErr := l.svcCtx.CultrueDbStruct.SyActivityDao.GetInfo(whereActivity)
	if activityErr != nil {
		returnData.Code = 100002
		returnData.Message = "查询秒杀活动信息错误"
		return returnData, activityErr
	}
	if !activityHas {
		returnData.Code = 100003
		returnData.Message = "未查询到当前的秒杀活动信息"
		return returnData, nil
	}
...
// 删除秒杀活动
func (l SeckillActivityLogic) ActivityDeleteLogic(req activityReq.ActivityDeleteReq, userId int) (resp *resp.CommonReply, err error) {
	returnData := reponse.ReturnStruct()
	adminRoleServie := serv.NewAdminUserRoleService(l.ctx, l.svcCtx)
	userRole := adminRoleServie.AmdinRole(userId)
	if userRole != 1 {
		returnData.Code = 100001
		returnData.Message = "当前用户角色不可以进行删除秒杀活动"
		return returnData, nil
	}
	whereActivity := []interface{}{"id = ? and is_delete = ? and activity_type = ?", req.ActivityId, 1, seckillActivityType}
	activityHas, _, activityErr := l.svcCtx.CultrueDbStruct.SyActivityDao.GetInfo(whereActivity)
